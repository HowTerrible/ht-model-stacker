...
| 店铺 | Shop | 购买渠道对应的线上 / 线下店铺；当前仅保留概念与文案，ts / 表结构待后续补充 |
...
2. **用户提交经审核入库**：用户可从「我的提交」（Submission）提交完整的厂家 / 产品条目，管理员审核通过后合并进资料库，并回填关联；
3. **用户申请补充条目**：在新增表单中，若用户输入的关联项（如厂家、产品等）在资料库中不存在，可通过表单内按钮提交「资料补充申请」（`DataSupplementRequest`），由管理员补充建库后自动回填绑定。

> 店铺（Shop）已纳入资料库概念范畴，但当前仅保留定义与文案，其 ts / 表结构待后续补充。
