...
面向「堆积人」的产品管理与统计工具：管理已购买的所有物品（不限于比例模型 / 工具辅料，可包含日用品、五金等），查询厂家、产品、店铺、文章，以及价格走向与历史最低价，记录堆积与烂尾。
...
堆积（Stack）与共享资料库（Product / Manufacturer / Shop / Article，即产品 / 厂家 / 店铺 / 文章）**不强绑定**：
...
资料库中的产品、厂家、文章等数据通过**两种形式**入库：

1. **后台直接维护**：管理后台（`apps/admin`）对厂家、产品、价格等进行增删改查，直接落库；
2. **用户提交经审核入库**：用户可从「我的提交」（Submission）提交完整的厂家 / 产品条目，或在堆积场景一键发起「资料补充申请」，管理员审核通过后合并进资料库，并回填关联。
