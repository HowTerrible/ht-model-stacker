...
面向「堆积人」的产品管理与统计工具：管理已购买的所有物品（不限于比例模型 / 工具辅料，可包含日用品、五金等），查询资料库，追踪价格走向与历史最低价，记录堆积与烂尾。
...
| 资料库 | Database | 产品 / 厂家 / 店铺 / 文章的集合，包含产品详情、历史价格、竞品关联、文章等；内容由后台直接维护，或由用户提交经审核后入库；与堆积不强绑定 |
| 资料补充申请 | DataSupplementRequest | 用户添加堆积时，若资料库中无对应产品 / 厂家，一键提交的补库申请；审核建库后自动绑定到来源堆积 |
| 资料提交 | Submission | 用户提交完整的厂家 / 产品条目，管理员审核通过后并入资料库（区别于堆积场景的「资料补充申请」，可提交完整资料） |
...
| 店铺 | Shop | 购买渠道对应的线上 / 线下店铺，包含图标、名称、外号、位置、平台（淘宝 / 京东 / 闲鱼 / 亚马逊 / 线下 / 其他）与数据更新时间 |
| 文章 | Article | 资料库中的富文本内容（产品趣闻、参考文献、制作窍门、评测科普等） |
| 价格走向 | Price Trend | 产品历史价格变化序列 |
| 历史最低价 | Lowest Price | 产品记录中的最低价格 |

## 堆积与资料库解耦

堆积（Stack）与共享资料库（Database）**不强绑定**：
...
> 该设计去掉了「加堆积必须同时审核资料」的强约束，低门槛记录购买行为，资料库内容可事后众包补全。

## 资料库数据来源

资料库数据通过以下**三种形式**入库：

1. **后台直接维护**：管理后台（`apps/admin`）对资料库数据进行增删改查，直接落库；
2. **用户提交经审核入库**：用户可从「我的提交」（Submission）提交完整的资料库条目，管理员审核通过后合并进资料库，并回填关联；
3. **用户申请补充条目**：在新增表单中，若用户输入的关联项（如厂家、产品等）在资料库中不存在，可通过表单内按钮提交「资料补充申请」（`DataSupplementRequest`），由管理员补充建库后自动回填绑定。

> 店铺（Shop）是资料库概念之一；产品 / 厂家 / 文章均可携带 `dataUpdatedAt`（数据更新时间），用于展示数据是否过期。
