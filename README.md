...
面向「堆积人」的产品管理与统计工具：管理已购买的比例模型 / 工具辅料，查询厂家、产品详情、价格走向与历史最低价，记录堆积与烂尾。
...
| 术语 | 英文 / Code | 说明 |
| ---- | ---- | ---- |
| 堆积 | Stack | 已购买但尚未开始制作的模型 / 工具辅料 |
| 堆积人 | Stacker | 购买堆积产品的用户，项目主要用户 |
| 产品 | Product | 模型与工具辅料的合称 |
| 比例模型 | Model | 按比例缩小的模型，含军事、民用、手办、战旗等类型 |
| 工具辅料 | Tool & Supply | 制作所需的工具与耗材 |
| 烂尾 | Wip | 开封但未做完的模型 |
| 制作阶段 | WipStage | 烂尾 / 制作进度的阶段描述（拆袋未组装 → … → 未喷保护漆） |
| 题材 | Theme | 树形分类结构，用于详细分类产品，也可分类工具辅料 |
| 竞品 | Competitor | 其他厂家同题材 / 同类产品，供堆积人对比参照 |
...
题材是树形结构，用于详细分类产品，既覆盖比例模型，也覆盖工具辅料：
...
1. **根层固定、枚举维护**：「模型 / 工具辅料」两级固定（可对齐 `ProductKind` / `ModelType` 枚举），保证导航一致性；往下允许 1~N 层，建议封顶 5~6 层。
